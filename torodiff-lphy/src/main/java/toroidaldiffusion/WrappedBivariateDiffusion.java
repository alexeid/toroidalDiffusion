...
//        double[] sigmaValues = new double[150]; // steps from 0.1 to 15
//        for (int s = 0; s < 150; s++) {
//            sigmaValues[s] = 0.1 + s * 0.1; // Increment by 0.1
//        }
//        for (double sigmaVal : sigmaValues) {
//            WrappedBivariateDiffusion diff = new WrappedBivariateDiffusion();
//            double[] muarr = {Math.PI, Math.PI * 0.5}; // mean of the diffusion
//            double[] sigmaarr = {sigmaVal, sigmaVal}; // Both sigma values set to current value
...
//        }
        ...
        for (int t = 0; t < N; t++) {
            ...
            double[] muT = diff.wntpdMu(Math.PI*0.5, Math.PI, t);
            //TODO std?
